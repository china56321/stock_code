...
#连续四天量增价增
  ...
  for trade_date in trade_dates:
    #在此处设定时间段范围。
    if trade_date in range(20211009,date_end):
        for i in range(a,b):
          ...
          if vol[i]/vol[i+4]>=2 and vol[i+1]/vol[i+4]>=2 and vol[i+2]/vol[i+4]>=2 and vol[i+3]/vol[i+4]>=2 \
            ...
            and stock_close[i+3]>=stock_open[i+3] and stock_close[0]<=ma30[0]*1.2 and pct_chg[0]>=3:
...
#   for trade_date in trade_dates:
#     #在此处设定时间段范围。
#     if trade_date in range(20211020,date_end):
#       for i in range(a,b):
